...
export default function ListingDetail() {
  ...
  const fetchListingAndRatingStatus = useCallback(async () => {
    ...
    try {
      ...
      // Ensure numeric fields are actually numbers (fixes toFixed crash)
      setListing({
        ...data,
                ...
                <div className="d-flex align-items-baseline gap-3 mb-4">
                  ...
                  {price > 1000 && (
                    <span className="fs-4 text-muted text-decoration-line-through">
                      K{Math.round((Number(price) || 0) * 1.15).toLocaleString()}
                    </span>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <div className="d-flex align-items-center gap-2 mb-2">
                    ...
                    <span className="text-secondary ms-2">
                      {(Number(average_rating) || 0).toFixed(1)} ({rating_count || 0}{' '}
                      {rating_count === 1 ? 'rating' : 'ratings'})
                    </span>
                  </div>
                </div>
